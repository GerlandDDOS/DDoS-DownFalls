...
from core import DDoS-DownFalls
from core import DDoS-DownFallsCollection

class ddos(GerlandDDOS):
    TITLE ="DDoS-DownFalls"
    ...
    INSTALL_COMMANDS = [
        ...
        "cd DDoS-DownFalls;sudo pip3 install -r requirements.txt"
    ]
    ...
    def run(self):
        ...
        os.system("cd DDoS-DownFalls;")
        subprocess.run([
            "sudo", "python3 DDoS-DownFalls", method, url, "socks_type5.4.1", threads, proxylist, multiple, timer])


class SaberDDOS(GerlandDDOS):
    TITLE = "SaberDDOS"
    DESCRIPTION = "SaberDDOS is basically an HTTP Denial of Service attack." \
                  "It send lots of HTTP Request"
    INSTALL_COMMANDS = ["sudo pip3 install SaberDDOS"]

    def run(self):
        target_site = input("Enter Target Site:- ")
        subprocess.run(["SaberDDOS", target_site])
